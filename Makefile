...
pokered_opt  = -jsv -k 01 -l 0x33 -m 0x13 -p 0xff -r 03 -t "POKEMON RED"
pokeblue_opt = -jsv -k 01 -l 0x33 -m 0x13 -p 0xff -r 03 -t "POKEMON BLUE"

%.gbc: $$(%_obj)
	rgblink -d -n $*.sym -m $*.map -l pokered.link -o $@ $^
	rgbfix $($*_opt) $@
	sort $*.sym -o $*.sym
